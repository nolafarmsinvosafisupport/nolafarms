...
export function NotificationBell() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch('/api/notifications', { method: 'PATCH' }).then(() => setUnread(0)).catch(() => undefined);
    }, 800);
  ...
  return (
    <div className="relative">
      <button type="button" aria-label="Notifications" onClick={() => setOpen(v => !v)}
        className="relative flex h-9 w-9 items-center justify-center rounded-full text-cream-secondary hover:text-cream-primary transition-colors">
        <Bell size={18} />
        {unread > 0 && (
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
              ...
              <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                ...
                {unread > 0 && <span className="rounded-full bg-red-500 px-2 py-0.5 text-[9px] font-bold text-white">{unread} new</span>}
              </div>
              <div className="p-2">
                {notifications.length === 0 ? (
                  ...
                  <div className="space-y-0.5">
                    {notifications.slice(0, 6).map(n => (
                      <div key={n.id} className={`flex items-start gap-2.5 rounded px-2 py-2.5 ${!n.read ? 'bg-white/5' : ''}`}>
                        {n.type === 'confirmed' ? <CheckCircle2 size={13} className="mt-0.5 flex-shrink-0 text-brand-leaf" />
                          : n.type === 'rejected' ? <XCircle size={13} className="mt-0.5 flex-shrink-0 text-red-400" />
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <span className="flex-shrink-0 text-[10px] text-cream-secondary/40">{fmtDate(n.created_at)}</span>
                          </div>
                          <p className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-cream-secondary/55">{n.message}</p>
                        </div>
                      </div>
                    ))}
                  </div>
